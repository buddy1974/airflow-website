import Link from 'next/link'
import Logo from './Logo'

export default function Footer() {
  return (
    <footer style={{ backgroundColor: '#2D2D2D' }} className="text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Column 1: Logo + tagline */}
          <div>
            <div className="mb-4">
              <div className="flex items-center gap-2">
                <svg width="32" height="32" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <circle cx="50" cy="50" r="8" fill="#00ABA8" />
                  <line x1="50" y1="42" x2="50" y2="10" stroke="#00ABA8" strokeWidth="2" />
                  <circle cx="50" cy="8" r="4" fill="#00ABA8" />
                  <line x1="50" y1="42" x2="20" y2="20" stroke="#00ABA8" strokeWidth="2" />
                  <circle cx="18" cy="18" r="4" fill="#00ABA8" />
                  <line x1="50" y1="42" x2="80" y2="20" stroke="#00ABA8" strokeWidth="2" />
                  <circle cx="82" cy="18" r="4" fill="#00ABA8" />
                  <line x1="50" y1="42" x2="15" y2="45" stroke="#00ABA8" strokeWidth="2" />
                  <circle cx="13" cy="45" r="4" fill="#00ABA8" />
                  <line x1="50" y1="42" x2="85" y2="45" stroke="#00ABA8" strokeWidth="2" />
                  <circle cx="87" cy="45" r="4" fill="#00ABA8" />
                  <line x1="50" y1="58" x2="50" y2="95" stroke="#00ABA8" strokeWidth="3" />
                </svg>
                <div className="flex flex-col leading-tight">
                  <span style={{ fontStyle: 'italic', fontSize: '1.4rem', color: '#ffffff', lineHeight: 1.1 }}>airflow</span>
                  <span style={{ fontSize: '0.9rem', color: '#00ABA8', fontWeight: 700, lineHeight: 1.1 }}>Fachpflegedienst</span>
                </div>
              </div>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed">
              Professionelle ambulante Beatmungspflege in Krefeld und Umgebung. Rund-um-die-Uhr für beatmungspflichtige Patienten da.
            </p>
          </div>

          {/* Column 2: Contact info */}
          <div>
            <h3 className="text-white font-semibold mb-4">Kontakt</h3>
            <ul className="space-y-2 text-gray-400 text-sm">
              <li>Stephanstraße 7</li>
              <li>47799 Krefeld</li>
              <li className="pt-1">
                <a href="tel:021516599998" className="hover:text-teal transition-colors">
                  Tel: 02151 / 65 99 998
                </a>
              </li>
              <li>
                <a href="tel:015174227338" className="hover:text-teal transition-colors">
                  Mobil: 0151 / 74 227 338
                </a>
              </li>
              <li>
                <a href="mailto:air.flow@gmx.de" className="hover:text-teal transition-colors">
                  air.flow@gmx.de
                </a>
              </li>
              <li className="pt-1">
                <a
                  href="https://wa.me/4915174227338"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-green-400 hover:text-green-300 transition-colors"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                  WhatsApp
                </a>
              </li>
            </ul>
          </div>

          {/* Column 3: Quick links */}
          <div>
            <h3 className="text-white font-semibold mb-4">Schnelllinks</h3>
            <ul className="space-y-2">
              {[
                { href: '/ueber-uns', label: 'Über uns' },
                { href: '/leistungen', label: 'Leistungen' },
                { href: '/beatmungspflege', label: 'Beatmungspflege' },
                { href: '/kontakt', label: 'Kontakt' },
                { href: '/stellenangebote', label: 'Stellenangebote' },
                { href: '/datenschutz', label: 'Datenschutz' },
                { href: '/impressum', label: 'Impressum' },
                { href: '/barrierefreiheit', label: 'Barrierefreiheit' },
              ].map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-gray-400 text-sm hover:text-teal transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </div>

        <div className="mt-10 pt-6 border-t border-gray-700 text-center text-gray-500 text-sm">
          © 2026 airflow Fachpflegedienst · Alle Rechte vorbehalten
        </div>
      </div>
    </footer>
  )
}
